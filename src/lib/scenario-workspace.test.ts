...
import { ensureScenarioTemplates, importWorkspaceScenarios } from './scenario-workspace'
...
it('adds source scenarios once and preserves saved assignments and deleted templates', () => {
  ...
  expect(workspace.layouts[0].flexSpotsEnabled).toBe(true)
  expect(workspace.layouts.slice(1).every((layout) => !layout.flexSpotsEnabled)).toBe(true)
...
it('imports six player sets and changes shared keys only when requested', () => {
  ...
  for (const player of PLAYER_TEMPLATES) {
    ...
    expect(imported.layouts.slice(40).every((layout) => !layout.flexSpotsEnabled && layout.slots.every((slot) => !slot.flex && slot.items.length <= 1))).toBe(true)
  }
