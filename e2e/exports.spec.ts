...
test('saves scenario layouts, shares keys, customizes destinations, and downloads working binary files', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Scenario name' }).fill('Fortress layout')
  ...
  await page.getByLabel('mpk assigned scenario').selectOption({ label: 'Fortress layout' })
  ...
  await page.reload()
  await page.locator('.scenario-trigger').click()
  await expect(page.locator('.scenario-option')).toHaveCount(41)
  await page.keyboard.press('Escape')
  await page.getByRole('button', { name: 'Export', exact: true }).click()
  await page.getByLabel('Preset barrel', { exact: true }).selectOption('mpk:2')
  await expect(mpk.getByLabel('Export slot 1', { exact: true })).toContainText('Obsidian × 12')
  await expect(page.getByLabel('mpk assigned scenario').locator('option:checked')).toHaveText('Fortress layout')
